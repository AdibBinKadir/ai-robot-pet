...
class DemoTTS:
    ...
    def _get_api_key(self):
        ...
        # Try environment variable first (both variations)
        api_key = os.environ.get('ELEVENLABS_API_KEY') or os.environ.get('ELEVEN_LABS_API_KEY')
        if api_key:
            return api_key
        
        # Try env files in multiple locations (.env and keys.env)
        env_files = ['.env', 'keys.env', 'backend/.env', 'backend/keys.env', '../.env', '../keys.env']
            ...
            try:
                if os.path.exists(env_file):
                    with open(env_file, 'r') as f:
                        for line in f:
                            line = line.strip()
                            if line.startswith('ELEVENLABS_API_KEY=') or line.startswith('ELEVEN_LABS_API_KEY='):
